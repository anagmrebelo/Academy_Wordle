...
export function Keyboard({
	...
	const isPerfectMatch = (oneKeyboardLetterValue: string, solutionWord: string, oneAttempt: string): boolean => {
		const solutionIndexes = findIndexesOfLetter(oneKeyboardLetterValue, solutionWord);
		const attemptIndexes = findIndexesOfLetter(oneKeyboardLetterValue, oneAttempt);
	...
	const changeColorsKeyboard = () => {
		const colorUpdateKeyboardLetters: KeyBoardLetter[] = keyboardLetters.map((oneKeyboardLetter) => {
			if (attempts[attemptNb].includes(oneKeyboardLetter.value)) {
				const colorUpdateKeyboardLetter = {...oneKeyboardLetter};
				if (isPerfectMatch(oneKeyboardLetter.value, solutionWord, attempts[attemptNb])) {
    ...
    const keyboardButtons = keyboardLetters.map((oneKeyboardLetter) => (
        <button
            ...
            className={ "keyboard-letter-sqr " + oneKeyboardLetter.color + "-btn"}
